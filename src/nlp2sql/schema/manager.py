...
from ..ports.schema_strategy import SchemaStrategyPort, SchemaContext
from ..ports.cache import CachePort
from ..core.entities import DatabaseType
from .analyzer import SchemaAnalyzer, RelevanceScore
from .embedding_manager import SchemaEmbeddingManager
from ..config.settings import settings
from ..exceptions import SchemaException, TokenLimitException
...
class SchemaManager:
    ...
    async def initialize(self, database_type: DatabaseType) -> None:
        ...
        try:
            ...
            raise SchemaException(f"Schema initialization failed: {str(e)}")
    
    async def get_optimal_schema_context(self, query: str, 
    ...
    async def find_relevant_tables(self, query: str, 
                                 database_type: DatabaseType,
                                 max_tables: int = 10) -> List[Tuple[str, float]]:
        """Find tables most relevant to a query."""
        return await self._find_relevant_tables(query, database_type, max_tables)
    
    async def get_table_relationships(self, table_name: str, 
                                    database_type: DatabaseType) -> List[Dict[str, Any]]:
        ...
        semantic_relations = await self.embedding_manager.find_related_tables(
            table_name, database_type
        )
    ...
    async def _find_relevant_tables(self, query: str, 
                                  database_type: DatabaseType,
                                  max_tables: int = 10) -> List[Tuple[str, float]]:
        """Find relevant tables using multiple strategies."""
        # Check cache
        ...
        for element, score in embedding_results:
            if element.get('type') == 'table':
                table_scores[element['name']] = max(
                    table_scores.get(element['name'], 0), score
                )
                ...
                if table_name:
                    table_scores[table_name] = max(
                        table_scores.get(table_name, 0), score * 0.8
                    )
    ...
    async def _refresh_schema_if_needed(self) -> None:
        ...
        if (self._last_refresh is None or 
            datetime.now() - self._last_refresh > self.refresh_interval):
    ...
    def _estimate_complexity(self, query: str, query_tokens: List[str]) -> float:
        ...
        complexity_indicators = {
            ...
            'aggregations': len([t for t in query_tokens if t.lower() in 
                               ['sum', 'count', 'avg', 'max', 'min', 'group']]) * 0.15,
            'subqueries': query.lower().count('select') * 0.1,
            'conditions': len([t for t in query_tokens if t.lower() in 
                             ['where', 'having', 'and', 'or']]) * 0.1
        }
        
        return min(sum(complexity_indicators.values()), 1.0)
    
    def _suggest_strategies(self, complexity_score: float, 
                          similar_elements: int) -> List[str]:
        ...
        for table in tables:
            ...
            if self.exclude_system_tables:
                ...
                system_patterns = [
                    ...
                    'sys_', 'sqlite_', 'mysql.', 'performance_schema'
                ]
